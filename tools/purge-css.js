...
/**
 * Get all HTML|JS files inside a directory
 * @param {string} dirPath - Path to the directory containing HTML|JS files
 * @returns {string[]} - Array of absolute paths to HTML|JS files
 */
const getHtmlJsFiles = (dirPath) => {
  return fs.readdirSync(dirPath)
    .filter(file => file.endsWith('.html') || file.endsWith('.js')) // Filter only HTML|JS files
...
const purgeCss = async (cssFilePath, htmlDirPath, outputCssFilePath) => {
  try {
    ...
    const htmlFiles = getHtmlJsFiles(htmlDirPath); // Get all HTML files
